...
const showScrollProgressionAmusement = () => {
    if(container.scrollTop < window.innerHeight){
        ...
        percentage1 = (container.scrollTop - window.innerHeight) / (amusementSector.offsetHeight - window.innerHeight)
    ...
    else if(container.scrollTop < amusementSector.offsetHeight + window.innerHeight){
    ...
    else if(container.scrollTop < amusementSector.offsetHeight + hauntedSector.offsetHeight){
        percentage2 = (container.scrollTop - window.innerHeight - amusementSector.offsetHeight) / (hauntedSector.offsetHeight - window.innerHeight)
    ...
    else if(container.scrollTop < amusementSector.offsetHeight + window.innerHeight + hauntedSector.offsetHeight){
        // container.removeEventListener('scroll', showScrollProgressionAmusement)
        hauntedText.style.strokeDashoffset =  0
        aquariumsText.style.strokeDashoffset =  aquariumsTextLength
    }

    else if(container.scrollTop < amusementSector.offsetHeight + hauntedSector.offsetHeight + aquariumsSector.offsetHeight){
        percentage3 = (container.scrollTop - window.innerHeight - amusementSector.offsetHeight - hauntedSector.offsetHeight) / (aquariumsSector.offsetHeight - window.innerHeight)
